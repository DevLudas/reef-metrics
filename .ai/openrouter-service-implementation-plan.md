...
```typescript
const openRouterService = new OpenRouterService(import.meta.env.OPENROUTER_API_KEY, "anthropic/claude-3.5-sonnet");
```
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (error instanceof OpenRouterError) {
      return errorResponse(error.code as any, error.message, error.statusCode || 500);
    }
...
export class AquariumAIService {
  ...
  constructor() {
    this.openRouter = new OpenRouterService(import.meta.env.OPENROUTER_API_KEY, "anthropic/claude-3.5-sonnet");
  }
...
Consider the interplay between different parameters.`;
  ...
  private buildAnalysisPrompt(parameters: ParameterStatusViewModel[], aquariumType: string, volume?: number): string {
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (error instanceof OpenRouterError) {
      ...
      return errorResponse("INTERNAL_SERVER_ERROR", "AI analysis failed. Please try again later.", 500);
    }
