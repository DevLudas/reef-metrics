...
describe('Parameter Status Utils', () => {
	describe('calculateStatus', () => {
		it('should return no_data when current value is null', () => {
			const result = calculateStatus(null, 1.020, 1.030);
		...
		it('should return normal status when deviation is less than 10%', () => {
			const result = calculateStatus(1.030, 1.020, 1.040);
			expect(result.status).toBe('normal');
	...
	it('should handle very small ranges', () => {
		const deviation = calculateDeviation(1.026, 1.020, 1.025);
		...
		it('should handle decimal precision', () => {
			const deviation = calculateDeviation(1.0305, 1.020, 1.030);
