...
const createMockChain = (returnValue: unknown = null, returnError: unknown = null): MockChain => {
  ...
  const chain: MockChain = {
    ...
    insert: vi.fn(function (this: MockChain) {
      return this;
    }),
    update: vi.fn(function (this: MockChain) {
      return this;
    }),
...
describe("AquariumService", () => {
  ...
  describe("createAquarium", () => {
    ...
    it("should throw AQUARIUM_TYPE_NOT_FOUND when type does not exist", async () => {
      ...
      // Act & Assert
      await expect(service.createAquarium(userId, command)).rejects.toThrow("AQUARIUM_TYPE_NOT_FOUND");
    });
    ...
    it("should throw DUPLICATE_AQUARIUM_NAME on unique constraint violation", async () => {
      ...
      // Act & Assert
      await expect(service.createAquarium(userId, command)).rejects.toThrow("DUPLICATE_AQUARIUM_NAME");
    });
  ...
  describe("updateAquarium", () => {
    ...
    it("should validate aquarium type when being updated", async () => {
      ...
      // Act & Assert
      await expect(service.updateAquarium(userId, aquariumId, command)).rejects.toThrow("AQUARIUM_TYPE_NOT_FOUND");
    });
    ...
    it("should throw NOT_FOUND when aquarium does not exist", async () => {
      ...
      // Act & Assert
      await expect(service.updateAquarium(userId, aquariumId, command)).rejects.toThrow("NOT_FOUND");
    });
  });
});
