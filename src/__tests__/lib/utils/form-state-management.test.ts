 ...
 */
export function shouldDisableForm(isSubmitting: boolean, isLoadingTypes: boolean, isValidating?: boolean): boolean {
 ...
 */
export function getFormStatusMessage(isLoadingTypes: boolean, selectedTypeId: string | undefined): string | null {
  if (isLoadingTypes) {
  ...
  describe("Form Header and Description", () => {
    ...
    describe("getFormDescription", () => {
      ...
      it("should return creation description when creating", () => {
        ...
        // Assert
        expect(description).toBe("Create a new aquarium to start tracking water parameters.");
      });
    });
